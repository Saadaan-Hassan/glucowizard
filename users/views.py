...
def google_auth(request):
    ...
    """
    frontend_callback = request.query_params.get("redirect_to", "http://localhost:3000/auth/callback")
...
def google_callback(request):
    ...
    try:
        ...
        user, created = User.objects.get_or_create(
            email=email,
            defaults={'username': user_info.user_metadata.get('username') or email.split('@')[0]}
        )
